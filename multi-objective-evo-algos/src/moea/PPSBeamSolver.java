...
public class PPSBeamSolver
{
  ...
  {
    NondominatedPopulation result = new Executor().withProblem("moea.PPSBeamProblem").withAlgorithm("NSGAII")
        .withMaxEvaluations(10000).distributeOnAllCores().run();

    int COUNT = result.size();
    float[][] data = new float[2][COUNT];
    int i = 0;
    System.out.format("Fundamental Frequency  Cost%n");
    ...
    Analyzer analyzer = new Analyzer().withProblem("moea.PPSBeamProblem").includeAllMetrics()
        .showStatisticalSignificance();

    Executor executor = new Executor().withProblem("moea.PPSBeamProblem").withMaxEvaluations(10000);
